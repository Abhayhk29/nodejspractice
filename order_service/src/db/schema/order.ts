...
export const orders = pgTable("orders",{
    ...
    orderNumer: integer("order_number").notNull().unique(),
    customerId: integer("customer_id").notNull(),
    amount: numeric("amount").notNull(),
    status: numeric("status").notNull(),
    taxId: numeric("tax_id").notNull(),
...
export const orderLineItems = pgTable("order_line_items",{
    id: serial("id").primaryKey(),
    itemName: varchar("item_name").notNull().unique(),
    qty: integer("qty").notNull(),
    price: numeric("price").notNull(),
    orderId: serial("order_id")
    .references(() => orders.id, {onDelete: "cascade"}).notNull(),
    createdAt: timestamp("created_at").notNull().defaultNow(), 
    updatedAt: timestamp("updated_at").notNull().defaultNow(), 
})
